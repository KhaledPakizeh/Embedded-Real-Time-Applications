...
static void LCD_initSPI()
{
   ...
   P10DIR |= BIT1 | BIT2 | BIT4 | BIT5;
    ...
    // MST MODE / SMCLK / inActive State is high/ MSB first
...
void LCD_DrawRectangle(int16_t xStart, int16_t xEnd, int16_t yStart, int16_t yEnd, uint16_t Color)
{
    ...
    if(xStart < MIN_SCREEN_X){xStart = 0;}
    if(xStart > MAX_SCREEN_X){xStart = 320;}
    if(yStart < MIN_SCREEN_Y){yStart = 0;}
    if(yStart > MAX_SCREEN_Y){yStart = 240;}

    if(xEnd < MIN_SCREEN_X){xEnd = 0;}
    if(xEnd > MAX_SCREEN_X){xEnd = 320;}
    if(yEnd < MIN_SCREEN_Y){yEnd = 0;}
    if(yEnd > MAX_SCREEN_Y){yEnd = 240;}
    ...
    /* Send out data only to the entire Screen_Tot */
    Screen_Tot = ((xEnd - xStart) + 1) * ((yEnd - yStart) + 1);

    SPI_CS_LOW;
    LCD_Write_Data_Start();
    for(int i = 0; i < Screen_Tot; i++)
    {
...
void LCD_Text(uint16_t Xpos, uint16_t Ypos, uint8_t *str, uint16_t Color)
{
    ...
    /* Set Screen_Tot back to span the entire LCD */
...
void LCD_Clear(uint16_t Color)
{
    ...
    LCD_Write_Data_Start();
    for(int span = 0; span < SCREEN_SIZE; span++)
    {
        LCD_Write_Data_Only(Color);
    }
    SPI_CS_HIGH;
...
void LCD_SetPoint(uint16_t Xpos, uint16_t Ypos, uint16_t color)
{
    ...
    if(Xpos < MIN_SCREEN_X){Xpos = 0;}
    if(Xpos > MAX_SCREEN_X){Xpos = 320;}
    if(Ypos < MIN_SCREEN_Y){Ypos = 0;}
    if(Ypos > MAX_SCREEN_Y){Ypos = 240;}
...
{
    /* Send out MSB */
    SPI_transmitData(EUSCI_B3_BASE, data >> 8);          /* TOP Byte first B15-B8*/
    while(EUSCI_B_SPI_isBusy(EUSCI_B3_BASE));

    /* Send out LSB */
    SPI_transmitData(EUSCI_B3_BASE, data & 0xFF);        /* Low Byte second B7-B0 */
    while(EUSCI_B_SPI_isBusy(EUSCI_B3_BASE));
}
...
void LCD_Init(bool usingTP)
{
    ...
    LCD_WriteReg(DISPLAY_CONTROL_3, 0x0000); /* set non-display Screen_Tot refresh cycle ISC[3:0] */
...
/*******************************************************************************
 * Function Name  : TP_ReadLocationXY
...
Point TP_ReadXY()
{
    ...
    // We get MSB or  7 bits of 12-bit ADC first
    // All on page 23 of XPT2046
    // bit 7  = bit 12
    X_val = SPISendRecvByte(0) << 5;
    // We get lower 5 bits of 12-bit ADC next
    // All on page 23 of XPT2046
    // bit 5 is now bit 2
    X_val |= SPISendRecvByte(0) >> 3;

    SPI_CS_TP_HIGH;


    LocationXY.x = ((X_val * MAX_SCREEN_X) / ADC_MAX_VAL);
    ...
    /* Read Y coord. */
    SPISendRecvByte(CHY);
    Y_val = SPISendRecvByte(0) << 5;
    Y_val |= SPISendRecvByte(0) >> 3;

    SPI_CS_TP_HIGH;

    LocationXY.y = ((Y_val * MAX_SCREEN_Y) / ADC_MAX_VAL);
